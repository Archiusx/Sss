...
// ── Bug 2 fix: full CRUD — get, update, delete ────────────────────────────────

/**
 * getInvestigation — fetch a single investigation by its Firestore doc ID.
 * Returns the normalised object (same shape as subscribeRecentInvestigations),
...
/**
 * updateInvestigation — merge partial fields into an existing investigation.
 * Only the caller-supplied fields are changed; everything else is preserved.
 * Allowed top-level fields: status, risk, summary, platforms, data.
 */
export async function updateInvestigation(user, firestoreId, fields = {}) {
  ...
  // updatedAt must be a server sentinel, not serialised JSON — set it after cleaning
  const ref = doc(db, "users", user.uid, "investigations", firestoreId);
...
/**
 * deleteInvestigation — permanently remove an investigation document.
 * The matching Firestore rule must allow delete (see firestore.rules fix).
 */
