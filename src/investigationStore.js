...
export async function saveInvestigation(user, investigation) {
  ...
  try {
    ...
    // CRITICAL: jsonClean() strips undefined/functions/symbols/NaN/Infinity and
    // converts any [key, value] array-pairs into {key, value} objects, because
    // Firestore rejects documents containing arrays nested directly inside
    // other arrays ("Nested arrays are not supported"). Skipping this step was
    // the root cause of saves silently failing.
    const cleanInvestigation = jsonClean(investigation);
