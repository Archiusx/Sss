...
async function scrapeReddit(username) {
  ...
  try {
    const data = await fetchJson(
      ...
      { headers: { "User-Agent": "CyIntel-OSINT/1.0" } }
    );
    const d = data?.data || {};
    return {
      platform: "Reddit",
      ...
      snippet: `Karma: ${(d.link_karma || 0) + (d.comment_karma || 0)} · Account age: ${d.created_utc ? new Date(d.created_utc * 1000).toLocaleDateString() : "Unknown"} · Verified: ${d.verified ? "Yes" : "No"}`,
      metadata: {
    ...
    return {
      ...
      title: `u/${username}`,
      url: `https://www.reddit.com/user/${username}/`,
      snippet: `Reddit API: ${error.message}`,
      extractor: "Reddit API",
    };
  }
}
...
async function scrapeTwitterX(username) {
  // Use Nitter public instance as scrape source via Jina
  ...
  try {
    ...
    return {
      ...
      snippet: snippet || "Twitter/X profile crawled.",
      metadata: {
    ...
    return {
      ...
      title: `@${username}`,
      url: `https://x.com/${username}`,
      snippet: `Twitter/X crawler failed: ${error.message}`,
      extractor: "Nitter/Jina Reader",
    };
  }
}
...
function baseMetadata(target, type) {
  ...
  const metadata = [
    ...
    ["Collection Mode", "Public web search + AI crawler + open-source APIs"],
    ["Platforms Scraped","Instagram, Telegram, Facebook, Reddit, Twitter/X, LinkedIn, TikTok, GitHub"],
