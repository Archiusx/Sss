...
import { saveInvestigation, subscribeRecentInvestigations, deleteInvestigation, updateInvestigation } from "./investigationStore";
...
// Static fallback is intentionally empty — all data is loaded from Firestore per-user.
// The dashboard shows an empty-state prompt until the real-time listener resolves.
...
function DashboardPage({ setActivePage, onStartInvestigation, onSelectInvestigation, investigation, investigationLoading, investigationError, recentItems, recentError, recentLoaded, onDeleteInvestigation, savingId, lastSavedId, user }) {
  ...
  const startSearch = async () => {
    ...
    if (!value) { setSearchError("Enter a target identifier first."); return; }
    setSearchError("");
  ...
  const handleDelete = async (e, inv) => {
    ...
    setDeletingId(inv.firestoreId);
    setDeleteError("");
    try {
      await onDeleteInvestigation(inv.firestoreId);
    } catch(err) {
  ...
  useEffect(() => {
    const t = window.setInterval(() => setRelativeClock(c => c + 1), 30000);
  ...
  const stats = [
    { label:"Total Investigations", value: isFirstLoad ? "—" : String(totalCount), delta: recentLoaded ? "Synced from Firestore" : "Loading…", icon:Target, color:"blue" },
    { label:"Suspects Identified",  value: isFirstLoad ? "—" : String(totalCount), delta:"Unique case targets",     icon:Users,         color:"indigo" },
    { label:"High Risk Cases",      value: isFirstLoad ? "—" : String(highRiskCount), delta:"Critical + high risk",  icon:AlertTriangle, color:"red"   },
    { label:"Platforms Scanned",    value: isFirstLoad ? "—" : String(platformCount), delta:"Across recent cases",   icon:Globe,         color:"cyan"  },
  ];
  const colorMap = {
      ...
      <div className="px-5 pt-5 pb-4">
        ...
        <div className="flex gap-3">
          <div className="flex-1 relative">
            ...
            <input ref={searchInputRef} value={searchVal} onChange={e => setSearchVal(e.target.value)}
              ...
              placeholder={searchTab==="image" ? "Paste a public image URL to investigate…" : `Search by ${searchTabs.find(t=>t.id===searchTab)?.label.toLowerCase()}…`}
          ...
          <button disabled={investigationLoading} onClick={startSearch}
            ...
            {investigationLoading ? "Running…" : "Investigate"}
          </button>
        </div>
        {searchError && <p className="mt-2 text-xs text-red-500">{searchError}</p>}
        {investigationError && !investigationError.includes("Firestore save failed") && <p className="mt-2 text-xs text-red-500">{investigationError}</p>}
        ...
        {savingId && !investigationLoading && (
          <div className="mt-3 rounded-lg px-3 py-2 text-xs text-amber-700 bg-amber-50 border border-amber-100 flex items-center gap-2">
            <Loader2 size={12} className="animate-spin"/>Saving case to Firestore…
          </div>
        )}
        ...
        <div className="flex items-center justify-between px-5 py-4" style={V.inner}>
          <div>
            ...
            <p className="text-xs mt-0.5" style={{ color:"var(--text-muted)" }}>
              {recentLoaded ? `${totalCount} case${totalCount===1?"":"s"} on record` : isFirstLoad ? "Loading from Firestore…" : "Firestore sync error"}
            </p>
          </div>
          <div className="flex items-center gap-2">
        ...
        {!isFirstLoad && (
          <div className="overflow-x-auto">
            <table className="w-full text-xs">
              <thead>
                <tr style={V.inner}>
                  {["Case ID","Target","Type","Risk","Status","Saved",""].map(hd =>
              ...
              <tbody>
                {liveItems.map(inv => {
                  ...
                  const isDeleting = deletingId === inv.firestoreId;
                  return (
                    <tr key={inv.firestoreId || inv.id}
        ...
        <div className="rounded-xl shadow-sm" style={V.card}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 p-4">{findings.length ? findings.map((p,i)=>{
            ...
            return <a key={`${p.platform}-${i}`} href={p.url} target="_blank" rel="noreferrer" className="rounded-xl p-3.5 cursor-pointer hover:shadow-sm transition-all" style={{ border:"1px solid #e2e8f0", backgroundColor:p.status==="found"?"#f8fff8":"#fafafa" }}>
              <div className="flex items-start justify-between mb-2"><div className="flex items-center gap-2"><PlatformPill abbr={p.abbr || (p.platform || "?").slice(0,2).toUpperCase()} color={p.color || "#2563eb"}/><span className="text-slate-700 text-xs font-semibold">{p.platform || "Unknown"}</span></div>{statusIcon(p.status)}</div>
  ...
  const handleStartInvestigation = async ({ target, type, redirectToOsint = true }) => {
    ...
    try {
      ...
      saveInvestigation(user, result)
        .then(() => {
          console.info("[CyIntel] Firestore save succeeded for case:", result.id);
        })
        .catch((saveErr) => {
          console.error("[CyIntel] Firestore save failed:", saveErr);
          // Show a visible banner — error is displayed globally above <main>
          setInvestigationError(
            "Firestore save failed: " +
            (saveErr?.message || String(saveErr)) +
            " — check your Firestore rules and auth state."
          );
        });
    } catch (error) {
