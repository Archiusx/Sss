...
const handleLogout = async () => {
  ...
  const handleStartInvestigation = async ({ target, type, redirectToOsint = true }) => {
    ...
    if (redirectToOsint) openWindow("osint");
    try {
  ...
  const nextZ = () => {
    let raised = 0;
    setZSeed(z => { raised = z + 1; return raised; });
    return raised || zSeed + 1;
  };
  const focusWindow = (id) => {
    const raised = nextZ();
    setActivePage(id);
    setWindows(ws => ws.map(w => w.id === id ? { ...w, minimized:false, z:raised } : w));
  };
  const openWindow = (id) => {
    const raised = nextZ();
    ...
    setWindows(ws => {
      ...
      if (existing) return ws.map(w => w.id === id ? { ...w, minimized:false, z:raised } : w);
