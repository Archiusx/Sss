...
export default function LoginPage() {
  ...
  async function handleGoogleLogin() {
    setLoad("google", true);
    try {
      // Mobile browsers block popups — use redirect on mobile, popup on desktop
      const isMobile = /Android|iPhone|iPad|iPod|Mobile/i.test(navigator.userAgent);
      if (isMobile) {
        // Redirect flow: page reloads, result is caught by getRedirectResult in main.jsx
        await signInWithRedirect(auth, googleProvider);
        return; // page will reload — no further code runs
      }
      const cred    = await signInWithPopup(auth, googleProvider);
      const user    = cred.user;
      const isNew   = cred._tokenResponse?.isNewUser ?? false;
      const payload = { authProvider: "google", email: cred.user.email, fullName: cred.user.displayName || "CyIntel Operative", phone: cred.user.phoneNumber || "", photoURL: cred.user.photoURL || "" };
      if (isNew) {
